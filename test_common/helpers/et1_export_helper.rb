    ...
    module Et1Export
      ...
      def calculated_claim_matchers(args)
        [
          ...
          "Alternative Respondent Address1: #{args.dig(:respondents)[0].dig(:work_address, :building)}",
          "Alternative Respondent Address2: #{args.dig(:respondents)[0].dig(:work_address, :street)}",
          "Alternative Respondent Address3: #{args.dig(:respondents)[0].dig(:work_address, :locality)}",
          "Alternative Respondent Address4: #{args.dig(:respondents)[0].dig(:work_address, :county)}",
          "Alternative Postcode: #{args.dig(:respondents)[0].dig(:work_address, :post_code)}",
          "Alternative Phone: #{args.dig(:respondents)[0].dig(:work_address, :telephone_number)}",
          '',
          '## Section 8: Your representative',
          '',
          "~8.1 Representative's name: #{args.dig(:representative[0], :name)}",
          "~8.2 Name of the representative's organisation: #{args.dig(:representative[0], :organisation_name)}",
          '~8.3 Address:',
          "Representative's Address 1: #{args.dig(:representative[0], :building)}",
          "Representative's Address 2: #{args.dig(:representative[0], :street)}",
          "Representative's Address 3: #{args.dig(:representative[0], :locality)}",
          "Representative's Address 4: #{args.dig(:representative[0], :county)}",
          "Representative's Postcode: #{args.dig(:representative[0], :post_code)}",
          "~8.4 Representative's Phone number: #{args.dig(:representative[0], :telephone_number)}",
          "Representative's Mobile number: #{args.dig(:representative[0], :alternative_telephone_number)}",
          "~8.5 Representative's Reference: #{args.dig(:representative[0], :dx_number)}",
          "~8.6 How would they prefer us to communicate with them?:",
          "Representative's E-mail address: #{args.dig(:representative[0], :email_address)}",
          "~8.7 Representative's Occupation: #{args.dig(:representative[0], :type)}",
          '',
