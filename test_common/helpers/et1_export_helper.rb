    ...
    module Et1Export
      ...
      def calculated_claim_matchers(args)
        [
          ...
          "~8.7 Representative's Occupation: #{factory_translate(args.dig(:representative, :type))}",
          '',
      ...
      def calculated_et1a_claim_matchers(args)
        ...
        group_claimants_for(args.dig(:user, :group_claims_csv)).each do |claimant|
          matchers.concat [
            '## Section et1a: claim',
            '',
            "~1.1 Title: #{factory_translate(claimant[:title])}",
