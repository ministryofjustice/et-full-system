        ...
        class EarningsSection < ::EtFullSystem::Test::FileObjects::Et3PdfFileSection::Base
          def has_contents_for?(response:)
            expected_values = {
              ...
              correct_pay_before_tax_period: response[:queried_pay_before_tax_period].to_s.split('.')[-2].camelcase,
              correct_take_home_pay: decimal_for(response[:queried_take_home_pay]),
              correct_take_home_pay_period: response[:queried_take_home_pay_period].to_s.split('.')[-2].camelcase,
