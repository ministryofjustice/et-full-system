Then(/^I can download the form and validate in PDF format$/) do
  within_admin_window do
    ...
    expect { api.claimants_api }.to eventually include(a_hash_including(first_name: @claimant[0].dig(:first_name))), timeout: 30, sleep: 2
...
Then(/^I can download the uploaded CSV data and validate in CSV format$/) do
  ...
  expect { atos_interface }.to eventually have_zip_file_containing(:et1_claim_csv_for, user: @claimant[0]), timeout: 30, sleep: 2
end
...
Then(/^I can download the form and validate in Rich Text format$/) do
  ...
  expect { atos_interface }.to eventually have_zip_file_containing(:et1_claim_rtf_for, user: @claimant[0]), timeout: 30, sleep: 2
end
...
Then("I can download the form and validate that the filename start with {string}") do |string|
  ...
  expect { atos_interface }.to eventually have_zip_file_containing(:et1_filename_start_with, user: @claimant[0], local_office: string), timeout: 30, sleep: 2
end
